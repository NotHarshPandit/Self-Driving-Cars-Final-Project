...
def GetCarModel(kappa_table, ts = None, ode = "cvodes"):
    if ts is None:
        ts = np.linspace(0, 0.025, 10)
    # system dimensions
...
def SimVehicle(y0, controller, integrator, car_dynamics, N_sim, MPC = False, x0_nlp_in = None, lamx0_nlp_in = None, lamg0_nlp_in = None, is_silent=False,ref_traj = None):
    ...
    if MPC:
        prob, N_mpc, n_x, n_g, n_p, lb_var, ub_var, lb_cons, ub_cons = controller(ref_traj)
        x0_nlp    = x0_nlp_in # np.zeros((n_x, 1))
        lamx0_nlp = lamx0_nlp_in # np.zeros((n_x, 1))
        lamg0_nlp = lamg0_nlp_in # np.zeros((n_g, 1))
        opts = {'ipopt.print_level': 0, 'print_time': 0, 'ipopt.max_iter': 100, 'ipopt.sb': 'yes'}
