...
def main():
    ...
    for traj_key in random_keys:
        ...
        mpc_traj_save_path = os.path.join(os.getcwd(), "mpc_trajectory", traj_key)
        os.makedirs(mpc_traj_save_path, exist_ok=True)
        np.save(os.path.join(mpc_traj_save_path, f"{traj_key}.npy"), x_log[:, 3:6])  # save only x, y, heading
        plot_actual_trajectory(trajectory, traj_key)
        mpc_trajectory = np.load(os.path.join(mpc_traj_save_path, f"{traj_key}.npy"))
